...
app.post('/movies', async (req, res) => {
  ...
  if (typeof newMovie.price !== 'number') {
    return res.status(400).json({ message: 'O preço precisa ser um número' });
  }
  if (typeof newMovie.movie !== 'string') {
    return res.status(400).json({ message: 'Movie precisa ser um texto' });
  }
  if (newMovie.price < 0) {
    return res.status(400).json({ message: 'O preço precisa ser maior que zero' });
}
  if (newMovie.movie.length === 0) {
    return res.status(400).json({ message: 'Movie precisa ser preenchido' });
  }
